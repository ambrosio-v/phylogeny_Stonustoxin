...
```{r}
fasta_obj.coi6A <- read.FASTA("COI_complete.fasta") # Im not really sure how this step is differnt but I've done a lot of over writing because I didn't want to make an new name for a data frame I'm gonna manipulate so Im just gonna do it
...
tips_to_drop <- setdiff(tree_coi_A6$tip.label, tips_to_keep)
write.tree(tree_coi_A6, file ="tree_coi_A6_single.tree")
```
